...
class BatchedMultiAgentEnv(gym.Env):
    def __init__(self, num_envs=1024):
        ...
        self.positions = torch.zeros(
            (num_envs, Config.NUM_AGENTS, 2), 
            dtype=torch.float32, 
            device=self.device
        )
        self.velocities = torch.zeros_like(self.positions)
        self.targets = torch.zeros((num_envs, 2), dtype=torch.float32, device=self.device)
        ...
        self.action_space = spaces.Box(
            low=-Config.MAX_ACCEL,
            high=Config.MAX_ACCEL,
            shape=(2,),
            dtype=np.float32
        )
    ...
    def _physics_step(self, accelerations):
        ...
        clamped_pos = torch.clamp(new_positions, 
    ...
    def step(self, actions):
        ...
        accelerations = torch.as_tensor(actions, device=self.device, dtype=torch.float32)
        ...
        dones, rewards = self._compute_terminal_rewards(rewards, collision_dones)
        self.velocities[dones] = 0.0
        # 奖励缩放
        ...
        return self._get_obs(), rewards.cpu().numpy(), dones.cpu().numpy(), info
    ...
    def _compute_base_rewards(self):
        ...
        # R1奖励：支持R0 + 保持距离
        rewards[:, 1] = (-r0_dist * Config.R1_R0_SCALE) + \
                       torch.norm(opp_positions - self.positions[:, 1].unsqueeze(1), dim=2).mean(dim=1) * Config.R1_OPPONENT_SCALE
        
        # R2/R3奖励：阻止R0 + 靠近R0
        rewards[:, 2] = (r0_dist * Config.R2_R0_SCALE) + \
                       (1 / (opp_dist[:, 0] + 1e-6)) * Config.R2_PROXIMITY_SCALE + \
                       Config.TIME_REWARD
        rewards[:, 3] = (r0_dist * Config.R3_R0_SCALE) + \
    ...
    def _handle_collision_penalties(self, rewards, violation_flags):
        """处理碰撞违规惩罚"""
        # 队伍掩码
        team_masks = torch.zeros((2, Config.NUM_AGENTS), dtype=torch.bool, device=self.device)
        team_masks[0, :2] = True  # 队伍1 (0,1)
        team_masks[1, 2:] = True  # 队伍2 (2,3)
        
        # 检测队伍违规
        team_violations = torch.stack([
            (violation_flags & mask).any(dim=1, keepdim=True)
    ...
    def _handle_boundary_penalties(self, rewards):
        ...
        boundary_penalty = self.boundary_collision_flags.float() * collision_speed * Config.WALL_PENALTY
    ...
    def _compute_terminal_rewards(self, rewards, collision_dones):
        ...
        # 终止条件
        dones = collision_dones | reach_target
    ...
    def reset(self):
        ...
        self._init_positions(
            x_ranges=[
                (0.0, left_x),                 
                (left_x + Config.BUFFER, Config.WIDTH)  
            ],
            y_range=(0.0, Config.HEIGHT)
        )
        # 目标点初始化
        self.targets[:, 0] = torch.rand(self.num_envs, device=self.device) * \
                            (Config.TARGET_X_RANGE[1] - Config.TARGET_X_RANGE[0]) + \
                            Config.TARGET_X_RANGE[0]
        self.targets[:, 1] = torch.rand(self.num_envs, device=self.device) * \
        ...
        for env_idx in range(self.num_envs):
            for attempt in range(max_attempts):
                ...
                for agent_idx in range(Config.NUM_AGENTS):
                    ...
                    positions[agent_idx, 0] = torch.rand(1, device=self.device) * (x_max - x_min) + x_min
                    positions[agent_idx, 1] = torch.rand(1, device=self.device) * (y_range[1] - y_range[0]) + y_range[0]
            ...
            if collision:
                ...
                angle = torch.linspace(0, 2 * np.pi, Config.NUM_AGENTS, device=self.device)
